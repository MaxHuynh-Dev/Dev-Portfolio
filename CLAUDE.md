...
is first declared — so undeclared it landed above `components` and beat
`.st-corgi`'s width: 120px, the file's intrinsic size, against a declared
4.2rem. Trap 1's cascade, arriving from a dependency.
...
src/components/Corgi/      the dog in the bottom chrome; corgi.json is
                           GENERATED by scripts/corgi-lottie.mjs
src/modules/Library/
...
- Eight things honour `prefers-reduced-motion` — the corgi, which never
  plays under it, and the CSS block, Lenis
